...
- 7 характеристик персонажа (Сила, Ловкость,  Следопытство, Интеллект, еда, вода)
...
└── Квартиры (базы данных):
 ...
 //создавали файл для того чтобы не вводить пароль в терминале постоянно

   ruslan@ruslan:~$ nano ~/.pgpass
ruslan@ruslan:~$ chmod 600 ~/.pgpass
ruslan@ruslan:~$ cat ~/.pgpass
localhost:5432:mudgame:muduser:mudpassword


Автоматизация удалений в БД:
"📋 Доступные команды:"
	make clean-db        -очистка БД(с подтверждением)"
	make clean-db-force  -очистка БД(без подтверждения)"
	make run             -запустить сервер"
    make git             -сохранение в гитхаб(+коммит внутри)
...
quit                       выход
craft                      создать...
use <цифра>                       использовать (цифра предмета в инвентаре)
quest                      показывает список квесто если они есть
buy <цифра>                купить у продавца
talk <имя>                       начать разговор
stop talk                  закончить разговор
all npc                    показывает всех npc
repair <цифра>             (у кузнеца) ремонт какой то вещи из списка
...
Отвечает за:
    ...
    Активацию свитков (шанс успеха,  )

    Идентификацию предметов, свитков, магических замков, свойств растений(В будущем)
...
Отвечает за:
    ...
    Сбор ресурсов (растения, грибы, минералы) — количество,
