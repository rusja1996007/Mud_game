📋 Обновлённый план на следующий урок:
1. ++Спавн монстров

    ++Респавн гоблина через N минут после смерти

    ++Таймер возрождения

2. Выбор цели в бою

    attack → атаковать текущего монстра

    attack 2 → атаковать второго монстра (для будущих данжей)

    Поддержка нескольких монстров в комнате

3. Улучшение входа в подземелье

    Путь до подземелья (5 минут, -5 еды, -5 воды)

    Рандомные предметы (цветы, грибы, травы) у входа

    Команда collect или pick для сбора

4. Блокировка данжа при занятости 🆕

    Если в данже есть другой игрок — войти нельзя

    Сообщение: "В подземелье уже кто-то есть. Подожди."

5. Побег из боя 🆕

    Команда flee или run

    Шанс успеха (например, 50%)

    При неудаче: монстр наносит бесплатный удар

    При успехе: возврат на вход в подземелье

    Шанс потерять предмет (например, 30% на случайный предмет из инвентаря)

6. Восстановление монстра после побега игрока 🆕

    Если игрок сбежал → здоровье монстра восстанавливается до максимума

    Если игрок умер → монстр тоже восстанавливается
## 🎮 Особенности игры
